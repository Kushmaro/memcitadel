...
# MemCitadel: the ChromaDB backend was removed (RFC 001 — Elasticsearch is the
# in-tree default). Upstream's chroma-only paths below are all gated behind
# _is_chroma_backend(), which can never be True here ("chroma" is not in the
# backend registry), so these placeholders exist only to keep those dead
# branches importable across upstream merges.
...
_HNSW_BLOAT_GUARD: dict = {}
_pin_hnsw_threads = None
...
def tool_reconnect():
    """Force the MCP server to drop cached backend + KnowledgeGraph state.

    Use after external scripts or CLI commands modify the palace database
    or replace ``knowledge_graph.sqlite3`` directly, which can leave cached
    handles stale (or, for server-mode backends like Elasticsearch, when the
    client needs to reestablish its connection).
    """
    global \
        _client_cache, \
        _collection_cache, \
