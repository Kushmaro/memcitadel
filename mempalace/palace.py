...
_DEFAULT_BACKEND = get_backend("elasticsearch")
...
def _open_collection_or_explain(
    ...
    *,
    ...
    try:
        ...
        return None
    # Server-mode backends (e.g. Elasticsearch) keep the palace in a remote
    # service — there are no local artifacts to detect, so the State B check
    # below would misreport every healthy palace as "no database yet".
    if not getattr(get_backend_class(backend_name), "server_mode", False):
